...
public sealed class RecipesController : ApiController
{
    ...
    [HttpPost]
    public async Task<IActionResult> GetAll(GetAllRecipeQuery request, CancellationToken token)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> Create(CreateRecipeCommand request, CancellationToken token)
    {
        var response = await _mediator.Send(request, token);
    ...
    [HttpPost]
    public async Task<IActionResult> DeleteById(DeleteRecipeByIdCommand request, CancellationToken token)
    {
        var response = await _mediator.Send(request, token);
