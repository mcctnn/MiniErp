...
public sealed class RecipeDetailsController : ApiController
{
    ...
    [HttpPost]
    public async Task<IActionResult> GetRecipeByIdWithDetails(GetRecipeByIdWithDetailsQuery request, CancellationToken token)
    {
    ...
    [HttpPost]
    public async Task<IActionResult> Create(CreateRecipeDetailCommand request, CancellationToken token)
    {
        var response = await _mediator.Send(request, token);
    ...
    [HttpPost]
    public async Task<IActionResult> DeleteById(DeleteRecipeDetailByIdCommand request, CancellationToken token)
    {
        var response = await _mediator.Send(request, token);
    ...
    [HttpPost]
    public async Task<IActionResult> Update(UpdateRecipeDetailCommand request, CancellationToken token)
    {
        var response = await _mediator.Send(request, token);
