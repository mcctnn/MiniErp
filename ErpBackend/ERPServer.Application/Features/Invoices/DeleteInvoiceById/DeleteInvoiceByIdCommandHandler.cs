    ...
    IUnitOfWork unitOfWork) : IRequestHandler<DeleteInvoiceByIdCommand, Result<string>>
{
    ...
    {
        Invoice invoice = await repository.GetByExpressionAsync(p => p.Id == request.Id, cancellationToken);

        if (invoice is null)
            return Result<string>.Failure("Fatura bulunamadı");

        List<StockMovement> stockMovement = await stockMovementRepository.
            Where(p => p.InvoiceId == invoice.Id).ToListAsync(cancellationToken);

        stockMovementRepository.DeleteRange(stockMovement);
