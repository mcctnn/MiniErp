    ...
    IUnitOfWork unitOfWork) : IRequestHandler<DeleteProductionByIdCommand, Result<string>>
{
    ...
    {
        Production? production = await repository.GetByExpressionWithTrackingAsync(p => p.Id == request.Id,cancellationToken);

        if (production is null)
            return Result<string>.Failure("Üretim bulunamadı");

        List<StockMovement> movements = await stockMovementRepository.
            Where(p => p.ProductionId == production.Id).ToListAsync(cancellationToken);
