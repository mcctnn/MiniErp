    ...
    IUnitOfWork unitOfWork) : IRequestHandler<RequirementsPlanningByOrderIdCommand, Result<RequirementsPlanningByOrderIdCommandResponse>>
{
    ...
    {
        Order? order = await repository.Where(p => p.Id == request.OrderId).
            Include(p => p.Details!).ThenInclude(p => p.Product).
            FirstOrDefaultAsync(cancellationToken);

        if (order is null)
            return Result<RequirementsPlanningByOrderIdCommandResponse>.Failure("Sipariş bulunamadı");

        List<ProductDto> uretilmesiGerekUrunListesi = new();
        ...
        if (order.Details is not null)
        {
            foreach (var item in order.Details)
            {
                ...
                List<StockMovement> movements = await stockMovementRepository.
                    Where(p => p.ProductId == product!.Id).ToListAsync(cancellationToken);
                ...
                if (stock < item.Quantity)
                {
                    ...
                    uretilmesiGerekUrunListesi.Add(uretilmesiGerekenUrun);
                }
            }

            foreach (var item in uretilmesiGerekUrunListesi)
            {
                Recipe? recipe = await recipeRepository.Where(p => p.ProductId == item.Id).
                    Include(p => p.Details!).ThenInclude(p => p.Product).FirstOrDefaultAsync(cancellationToken);
                ...
                if (recipe is not null && recipe.Details is not null)
                {
                    foreach (var detail in recipe.Details)
                    {
                        List<StockMovement> urunMovements = await stockMovementRepository.
                            Where(p => p.ProductId == detail!.ProductId).ToListAsync(cancellationToken);
        ...
        requirementsPlanningProducts = requirementsPlanningProducts.GroupBy(p => p.Id).Select(s => new ProductDto
        {
            Id = s.Key,
            Name = s.First().Name,
